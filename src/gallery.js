...
const getUsername = req => (req.user && req.user.profile.username) || 'humanhybrids';
...
function getTweets(req) {
  return new Promise((resolve, reject) => {
    ...
    const requests = [moreTweets(req, { q: '@MikePanoots filter:media -filter:retweets', count: 100, result_type: 'recent' })];
    if (req.user) {
      requests.push(cli.get('/favorites/list', { count: 200 }));
      requests.push(cli.get('/statuses/user_timeline', { count: 200, user_id: req.user.profile.id }));
    }
    Promise.all(requests).then(([tweets, favorites = [], timeline = []]) => {
      const retweets = timeline.filter(status => status.retweeted_status)
        .map(status => status.retweeted_status.id_str);
      resolve(transform(req, tweets, favorites.map(fav => fav.id_str), retweets));
    }).catch(reject);
  });
}
...
router.post('/like/:id', (req, res) => {
  if (req.user) {
    log.debug(`liking tweet with id ${req.params.id} on behalf of ${getUsername(req)}`);
    const cli = getClient(req);
    const id = req.params.id;
    cli.post('/favorites/create', { id })
...
router.post('/retweet/:id', (req, res) => {
  if (req.user) {
    log.debug(`retweeting tweet with id ${req.params.id} on behalf of ${getUsername(req)}`);
    const cli = getClient(req);
    const id = req.params.id;
    cli.post(`/statuses/retweet/${id}.json`, {})
      .then(() => cli.get(`/statuses/show/${id}`, {}))
      .then(result => res.json(mapStatus(result)))
      .catch(error => res.status(500).json({ error }));
  } else {
...
router.get('/favorited', (req, res) => {
  if (req.user) {
    ...
    getClient(req).get('/favorites/list', {})
